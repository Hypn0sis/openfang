...
pub fn builtin_tool_definitions() -> Vec<ToolDefinition> {
    vec![
        ...
        ToolDefinition {
            ...
            description: r#"Send a message or media to a user on a configured channel (email, telegram, slack, discord, etc). IMPORTANT: If recipient is empty string "", the system automatically uses the default channel from config. Examples:
- channel_send(channel='discord', message='ciao') → sends to default discord channel (uses default_channel_id from config)
- channel_send(channel='telegram', message='hello') → sends to default telegram chat (uses default_chat_id)
- channel_send(channel='discord', recipient='123456', message='hi') → sends to specific channel
- channel_send(channel='email', recipient='test@test.com', subject='Subject', message='Body') → sends email
- channel_send(channel='telegram', message='AI news today: [LLM will generate content]') → LLM generates dynamic content then sends

The message parameter can contain dynamic content generated by the LLM itself."#.to_string(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "channel": { "type": "string", "description": "Channel: 'email', 'telegram', 'discord', 'slack', etc." },
                    "recipient": { "type": "string", "description": "Optional. Leave empty '' to use default from config. Discord channel_id, Telegram user_id, or email address." },
                    "subject": { "type": "string", "description": "Optional subject for email" },
                    "message": { "type": "string", "description": "Message body - can be dynamic text generated by LLM" },
                    "image_url": { "type": "string", "description": "Optional image URL to send" },
                    "file_url": { "type": "string", "description": "Optional file URL to send" },
                    "file_path": { "type": "string", "description": "Optional local file path to send" },
                    "filename": { "type": "string", "description": "Optional filename for attachments" },
                    "thread_id": { "type": "string", "description": "Optional thread/topic ID" }
                },
